...
// Service Workers
//-----------------------------------------------------------------------
if (ADMIXAPP.PWA.enable) {
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('https://admix.scubeerp.in/service_worker.js')
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("ADMIXAPPAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("ADMIXAPPAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("ADMIXAPPDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (ADMIXAPP.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (ADMIXAPP.Dark_Mode.local_mode.enable) {
    var nightStart = ADMIXAPP.Dark_Mode.local_mode.start_time;
    var nightEnd = ADMIXAPP.Dark_Mode.local_mode.end_time;
...
if (ADMIXAPP.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("ADMIXAPPDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("ADMIXAPPDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("ADMIXAPPDarkmode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
function testMode() {
    ...
    console.clear();
    console.log("%cADMIXAPP", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(ADMIXAPP.PWA.enable, "PWA")
    testModeMsg(ADMIXAPP.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(ADMIXAPP.Dark_Mode.local_mode.enable, "Local dark mode (between " + ADMIXAPP.Dark_Mode.local_mode.start_time + ":00 and " + ADMIXAPP.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(ADMIXAPP.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(ADMIXAPP.RTL.enable, "RTL")
    testModeMsg(ADMIXAPP.Test.enable, "Test mode")
    testModeMsg(ADMIXAPP.Test.alert, "Test mode alert")
    ...
    testModeMsg(ADMIXAPP.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = ADMIXAPP.Test.word;
    var value = "";
